...
$componentRoot = Join-Path $Root 'implementations/go-http1-websocket'
$sourceRoot = Join-Path $componentRoot 'source'
& go -C $sourceRoot test -count=1 ./...
if ($LASTEXITCODE -ne 0) { throw 'go-http1-websocket tests failed.' }
$rid = switch ($RuntimeIdentifier) {
    'win-x64' { @{os='windows';arch='x64';goOs='windows';goArch='amd64';name='go-http1-websocket.exe'} }
    'linux-x64' { @{os='linux';arch='x64';goOs='linux';goArch='amd64';name='go-http1-websocket'} }
}
$stagingRoot = Join-Path $OutputRoot "go-http1-websocket/$RuntimeIdentifier"
$packageRoot = Join-Path $stagingRoot 'package'
$packageBin = Join-Path $packageRoot "bin/$RuntimeIdentifier"
Remove-Item -LiteralPath $stagingRoot -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force -Path $packageBin,(Join-Path $packageRoot 'implementations') | Out-Null
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'go-http1-websocket build failed.' }
} finally { $env:GOOS=$oldGoOs; $env:GOARCH=$oldGoArch }
Copy-Item (Join-Path $componentRoot 'protocol-lab-package.json') $packageRoot
Copy-Item (Join-Path $componentRoot 'implementations/go-http1-websocket.yaml') (Join-Path $packageRoot 'implementations')
$implementationPath = Join-Path $packageRoot 'implementations/go-http1-websocket.yaml'
