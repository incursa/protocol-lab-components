...
The controller also contains earlier immutable uploads from the Caddy, nginx, quic-go, aioquic, Kestrel, quiche, and ngtcp2 correction sequences. Do not use superseded or overbroad uploads for ranking. Use Caddy `0.1.9`, nginx `0.1.9`, quic-go HTTP/3 `0.1.8`, aioquic `0.3.3`, Kestrel `0.1.8`, quiche `0.1.10`, and ngtcp2 `0.1.9` for current h3spec/QPACK rows; use their cited older immutable versions only for retained payload or peer-characterization rows. The quiche current proof includes two explicit conformance failures and neither peer target has an accepted performance measurement.
