...
Package version `0.2.2` executes the seven strict `dns.doh3.*` identities plus authoritative `dns.doh3.interoperability.query.a`. It requires QUIC v1, TLS 1.3, ALPN `h3`, authenticated certificate proof, exact DoH authority/path/method/media binding, parsed DNS semantic parity, canonical response hashes, connection reuse, zero malformed, retry, failure, and timeout outcomes, and protocol, TLS, and certificate profiles bound to the selected strict or interoperability scenario.
