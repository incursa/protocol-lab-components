...
The clean component integration head is `2f8a8fc`; the runner integration head is `2caf947`, including the integration-only TLS record load-shape repair, exact DoQ admission, and HTTP/1.1 TLS WebSocket diagnostic admission. The consolidated runner and CLI build with zero warnings and errors, and the combined TLS, gRPC, secure-DNS, and WebSocket parser/admission/writer selection passes `274/274` focused tests. The component integration validates `54` public and `54` internal manifests; focused race tests and `go vet` pass for every consolidated Go executor and target.
