...
executor_version="${PLAB_EXECUTOR_VERSION:-0.1.8}"
...
docker run --rm ${docker_host_args[*]} "$image" curl --http3-only --max-time "$timeout_seconds" --insecure --silent --show-error --output /dev/null --write-out '%{http_code}' "$container_target_url"
EOF
...
status="$(docker run --rm "${docker_host_args[@]}" "$image" curl --http3-only --max-time "$timeout_seconds" --insecure --silent --show-error --output /dev/null --write-out '%{http_code}' "$container_target_url" > "$output_root/stdout.txt" 2> "$output_root/stderr.txt" && cat "$output_root/stdout.txt")"
