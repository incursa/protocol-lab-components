...
$componentRoot = Join-Path $Root 'executors/go-http1-websocket-executor'
$sourceRoot = Join-Path $componentRoot 'source'
& go -C $sourceRoot test -count=1 ./...
if ($LASTEXITCODE -ne 0) { throw 'go-http1-websocket-executor tests failed.' }
$rid = switch ($RuntimeIdentifier) {
    'win-x64' { @{os='windows';arch='x64';goOs='windows';goArch='amd64';name='go-http1-websocket-executor.exe'} }
    'linux-x64' { @{os='linux';arch='x64';goOs='linux';goArch='amd64';name='go-http1-websocket-executor'} }
}
$stagingRoot = Join-Path $OutputRoot "go-http1-websocket-executor/$RuntimeIdentifier"
$packageRoot = Join-Path $stagingRoot 'package'
$packageBin = Join-Path $packageRoot "bin/$RuntimeIdentifier"
Remove-Item -LiteralPath $stagingRoot -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force -Path $packageBin,(Join-Path $packageRoot 'test-executors') | Out-Null
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'go-http1-websocket-executor build failed.' }
...
Copy-Item (Join-Path $componentRoot 'test-executors/go-http1-websocket-executor.yaml') (Join-Path $packageRoot 'test-executors')
