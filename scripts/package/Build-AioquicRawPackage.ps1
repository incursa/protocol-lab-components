...
$sourceImplementationManifest = Join-Path $componentRoot 'implementations/aioquic-raw.yaml'
...
$sourceLicense = Join-Path $Root 'implementations/aioquic-http3/third-party/aioquic-LICENSE.txt'
$buildImage = 'incursa-protocol-lab-aioquic-raw-build:0.1.0'

foreach ($path in @($sourcePackageManifest, $sourceInternalManifest, $sourceImplementationManifest, $sourceRunSh, $sourceCert, $sourceKey, $sourceLicense, (Join-Path $sourceRoot 'server.py'), (Join-Path $sourceRoot 'requirements-build.txt'), (Join-Path $sourceRoot 'Dockerfile'))) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Required aioquic raw package input was not found: $path"
    }
}

& docker build --pull --tag $buildImage --file (Join-Path $sourceRoot 'Dockerfile') $Root
if ($LASTEXITCODE -ne 0) {
    throw "aioquic raw image build failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($containerId)) {
    throw 'Could not create aioquic raw build container.'
}
try {
    & docker cp "${containerId}:/out/bin/aioquic-raw" (Join-Path $packageBinLinux 'aioquic-raw')
    if ($LASTEXITCODE -ne 0) {
        throw "Could not extract aioquic raw build output from container $containerId."
    }
}
finally {
...
if (-not $SkipSmoke) {
    ...
    & docker run --rm --mount "type=bind,source=$smokeRoot,target=/package" --workdir /package `
        ...
        /package/bin/linux-x64/aioquic-raw --self-test
    if ($LASTEXITCODE -ne 0) {
        throw "aioquic raw extracted binary smoke failed with exit code $LASTEXITCODE."
    }
}
...
Copy-Item -LiteralPath $sourceLicense -Destination (Join-Path $packageLicenses 'aioquic-LICENSE.txt') -Force
...
Copy-Item -LiteralPath $sourceImplementationManifest -Destination (Join-Path $packageImplementations 'aioquic-raw.yaml') -Force
...
try {
    ...
    foreach ($entryName in @('README.md', 'bin/linux-x64/aioquic-raw', 'certs/leaf.pem', 'certs/leaf-key.pem', 'third-party/aioquic-LICENSE.txt', 'implementations/aioquic-raw.yaml', 'protocol-lab-package.json', 'protocol-lab.internal.json', 'run.sh')) {
