...
docker run --rm -v "$PWD/$output_root:/out" "$image" curl --http3-only --max-time "$timeout_seconds" --insecure --silent --show-error --output /out/body.bin --write-out '%{http_code}' "$target_url"
EOF
...
status="$(docker run --rm -v "$PWD/$output_root:/out" "$image" curl --http3-only --max-time "$timeout_seconds" --insecure --silent --show-error --output /out/body.bin --write-out '%{http_code}' "$target_url" > "$output_root/stdout.txt" 2> "$output_root/stderr.txt" && cat "$output_root/stdout.txt")"
