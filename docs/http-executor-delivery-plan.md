...
The clean component integration head is `ed9225b`; the runner integration head is `8c8d30b`, including the integration-only TLS record load-shape repair, exact DoQ admission, HTTP/1.1 TLS WebSocket diagnostic admission, and exact mutual-TLS admission. The consolidated runner and CLI build with zero warnings and errors. The mutual-TLS focused and widened regression gates pass `21/21`, `199/199`, and `51/51`; the preceding combined TLS, gRPC, secure-DNS, and WebSocket selection gate passed `274/274`. The component integration validates `59` public and `59` internal manifests; focused race tests and `go vet` pass for every consolidated Go executor and target.
