...
const (
	executorID           = "go-http1-websocket-executor"
	executorVersion      = "0.1.0"
	loadGeneratorID      = "go-http1-websocket-load"
...
var expectations = map[string]scenarioExpectation{
	"http1.websocket.rfc6455.cleartext.upgrade":        {id: "http1.websocket.rfc6455.cleartext.upgrade", operation: "establish", messageType: "none", payloadHash: hash(nil), connectionMetric: true},
	"http1.websocket.rfc6455.cleartext.text-echo":      {id: "http1.websocket.rfc6455.cleartext.text-echo", operation: "text-echo", messageType: "text", payload: []byte(textPayload), payloadHash: "504585b0bb4fd77012ea2575efbcdb58f4c33e6b543e9567a65896d213720c29", requestOpcode: 0x1, responseOpcode: 0x1},
	"http1.websocket.rfc6455.cleartext.binary-echo":    {id: "http1.websocket.rfc6455.cleartext.binary-echo", operation: "binary-echo", messageType: "binary", payload: repeatedByte(66, 1024), payloadHash: "9b6ce55f379e9771551de6939556a7e6b949814ae27c2f5cfd5dbeb378ce7c2a", requestOpcode: 0x2, responseOpcode: 0x2},
	"http1.websocket.rfc6455.cleartext.control-frames": {id: "http1.websocket.rfc6455.cleartext.control-frames", operation: "control-frames", messageType: "none", payload: []byte(controlPayload), payloadHash: "4848de689e96825e0e05b6c3e96e48f2ad7ec7805fb64ecf824ccc6aa9c58883", requestOpcode: 0x9, responseOpcode: 0xA},
	"http1.websocket.rfc6455.cleartext.close":          {id: "http1.websocket.rfc6455.cleartext.close", operation: "close", messageType: "none", payloadHash: hash(nil)},
}
...
func main() {
	target := flag.String("target-url", os.Getenv("PLAB_TARGET_BASE_URL"), "cleartext HTTP/1.1 WebSocket target URL")
	...
	preflight, err := performOperation(targetURL, expectation, timeout)
	...
	warmup := runPhase(targetURL, expectation, time.Duration(config.warmupSeconds)*time.Second, timeout, "warmup")
	...
	measured := runPhase(targetURL, expectation, time.Duration(config.durationSeconds)*time.Second, timeout, "measured")
...
func checkIdentityOrExit(output string) scenarioExpectation {
	...
	verifySubstitution("PLAB_PROTOCOL_VARIANT", "websocket-h1-cleartext-upgrade", "protocol variant")
	...
	if _, ok := knownUnsupported[scenario]; ok {
		document := map[string]any{
			...
			"executorId": executorID, "reason": "exact WebSocket identity is outside this cleartext RFC 6455 over HTTP/1.1 package",
		}
...
func runPhase(targetURL string, expectation scenarioExpectation, duration, timeout time.Duration, name string) phaseSummary {
	...
	for time.Now().Before(deadline) {
		proof, err := performOperation(targetURL, expectation, timeout)
		if err != nil {
			if isTimeout(err) {
...
func openWebSocket(targetURL string, timeout time.Duration) (*wsConnection, error) {
	...
	proof := handshakeProof{
		Binding: "http1-upgrade", Scheme: "ws", TransportSecurity: "cleartext", Endpoint: "/websocket", Authority: "websocket.plab.test",
		...
		FallbackDetected: response.ProtoMajor != 1 || response.ProtoMinor != 1,
	}
	var failures []string
...
func writePreflightArtifacts(output string, expectation scenarioExpectation, proof operationProof, operationErr error) {
	...
	validation := map[string]any{
		"scenarioId": expectation.id, "passed": passed, "requestedProtocol": "websocket-over-h1-cleartext",
		"observedProtocol": func() string {
			if proof.Handshake.ConnectionEstablished {
				return "websocket-over-h1-cleartext"
			}
			return ""
		}(),
	...
	writeRequired(output, "protocol-proof.json", map[string]any{
		...
		"protocolVariant": "websocket-h1-cleartext-upgrade", "fallbackDetected": proof.Handshake.FallbackDetected,
		"websocket": proof,
	})
...
func normalizeResult(expectation scenarioExpectation, config loadConfig, summary phaseSummary) normalizedResult {
	...
	return normalizedResult{
		...
		Warnings: []string{"Local package-backed WebSocket smoke is diagnostic and non-publishable. TLS, RFC 8441, RFC 9220, breadth diagnostics, WebTransport, and legacy websocket.echo substitution are unsupported."},
	}
}
...
func normalizeTarget(value string) (string, error) {
	...
	if parsed.Scheme != "http" || parsed.Host == "" {
		return "", errors.New("cleartext WebSocket target must use http://host:port")
	}
