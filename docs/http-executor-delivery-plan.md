...
| gRPC/H2 unary breadth | `grpc.h2.unary.empty`, `grpc.h2.unary.fixed-metadata`, `grpc.h2.unary.gzip`, `grpc.h2.unary.large`; `grpc-h2-contract-breadth-smoke`; `grpc-h2-diagnostic` | each `1 / 0 / 0` | scenario `567cd8926c695415133e7a089135fd857d8074944a1158d80b00011706c35174`; executor `e2c31ecb43488a7244bb54be90234f0c7a7d9934862cad503f36ab28ca790090`; target `9a3e7bc8126c91c79847693be0602bd1ae3a0c5def59b3b3fe95f590ea6f01c6` | `C:\shared\src\incursa\.artifacts\grpc-unary-breadth-runner-evidence\grpc-unary-breadth-clean-v2-direct-package-cell` |
...
Final Linux package SHA-256 values are: TLS executor `b9c5ad8a9e5d2eafb4be2d234c1590c03144bfcfa6a3c5aa4bab4f02069a9ec2`; gRPC `0.1.0` executor `2d87b50b6fc36e41f387db3d5a6d1ad6783dc0d62f25a7cb69de58dc4cc5ad4d`; gRPC `0.1.0` target `e8503edae14776ff1a1084d65dfb23b937675e0b1eab7d5222597777bd6d82e7`; gRPC unary-breadth `0.2.0` executor `ce470d35ed5e35ff34c24f03f99d8b7701703d9f88c5188a95a4f45825013211`; gRPC unary-breadth `0.2.0` target `3c403b02b4bbcc79615397d858255b0fe016f1ca16d4138d17ca9acf03e575b2`; DoT executor `769c567ba580eaf946f457482558c8cc939f5c9b059b715c2f2691b8b1e647c2`; DoT target `2fe9dc1171850f216856736eea63ec002ae308e63f0ee032c14904bda849bfe0`.

Runner admission is exact rather than family-substituting. Target manifests carry an exact `scenario.<id>` capability, and the runner requires that capability before using the package-executor role bridge. The TLS parser checks TLS version, cipher, key exchange, ALPN, certificate DER/SPKI identity, no resumption, no early data, zero application bytes, load echo, identities, outcomes, and artifacts. The gRPC parser additionally checks the committed service digest, HTTP/2/TLS proof, channel reuse, exact per-scenario service method/path, payload/protobuf/frame scopes, fixed ASCII and decoded binary metadata, gzip decompression and encoder provenance without claiming compressed-wire equivalence, status/media type/trailers, and all terminal counts. The DoT parser checks local-authoritative-only semantics, exact DNS question/answer/TTL, canonical 27/43-byte hashes, two-octet framing, message-ID correlation, TLS/ALPN identity, and separate malformed/retry/failure/timeout counts.

Explicit unsupported inventories remain fail closed. TLS: `tls.handshake.resumed`, `tls.0rtt`, `tls.record.throughput`, `tls.handshake.full.tls12`, `tls.handshake.full.chacha20`, `tls.handshake.mutual-auth`, `tls.early-data.accepted`, `tls.early-data.rejected`, `tls.key-update.diagnostic`, and `tls.record.coverage`. gRPC: `grpc.h2.bidi-streaming.echo`, `grpc.h2.client-cancellation`, `grpc.h2.client-streaming.echo`, `grpc.h2.deadline-exceeded`, `grpc.h2.server-streaming.echo`, `grpc.h2.trailers-only-status`, and `grpc.h2.unary.echo-new-channel`. DNS: `dns.classic.tcp.query.a`, `dns.classic.udp-truncated-tcp-retry`, `dns.classic.udp.query.a`, `dns.doh2.query.a`, `dns.doh3.get.a`, `dns.doh3.query.a`, `dns.doh3.query.aaaa`, `dns.doh3.query.cname-chain`, `dns.doh3.query.large-dnssec-shaped`, `dns.doh3.query.nodata`, `dns.doh3.query.nxdomain`, and `dns.doq.query.a`.

Verification commands included `go test -race -count=1 ./...` and `go vet ./...` for the H1, H2, TLS, gRPC, and DoT Go packages; warning-as-error builds for the affected .NET runner, CLI, lab, worker, and TLS target projects; `Validate-ProtocolLabComponentManifests.ps1`; every affected package builder and build-attestation validator; the direct package-backed runner commands; public repository health validation; Protocol Execution Result v2 JSON Schema plus artifact-hash validation; and `git diff --check`. The gRPC unary-breadth archives were rebuilt from clean component commit `3e238bc` and are parity-eligible; every local result remains diagnostic and ineligible for publication or ranking.
...
The `0.2.0` breadth slice adds exact `empty`, `fixed-metadata`, `gzip`, and
`large` identities while keeping the existing echo lane at `0.1.0`. The real
four-cell runner smoke completed each identity once with zero failed,
deadline-exceeded, cancelled, or timed-out operations. All `4/4` public result
schemas and `41/41` referenced artifact hashes passed. Gzip evidence records
semantic decompression, `wireBytesComparable: false`, `grpcFrameSha256: null`,
encoder provenance, and `80` observed compressed frame bytes; it does not
invent a canonical gzip wire digest.

```powershell
pwsh -NoProfile -File scripts\benchmarking\Invoke-ProtocolLabBenchmarkSet.ps1 `
  -ImplementationIds go-grpc-h2 `
  -ScenarioIds grpc.h2.unary.empty,grpc.h2.unary.fixed-metadata,grpc.h2.unary.gzip,grpc.h2.unary.large `
  -Protocol h2 `
  -OverrideLoadToolId go-grpc-h2-executor `
  -OverrideLoadToolMode process `
  -LoadProfileId grpc-h2-diagnostic `
  -ComponentPackageDirectory C:\shared\src\incursa\.worktrees\protocol-lab-components-grpc-unary-breadth\artifacts\grpc-unary-breadth-clean-packages `
  -ComponentPackage org.protocol-lab.components.implementation.go-grpc-h2.0.2.0.win-x64.plabpkg,org.protocol-lab.components.executor.go-grpc-h2-executor.0.2.0.win-x64.plabpkg,org.protocol-lab.components.scenario.grpc-h2-performance.0.2.0.plabpkg `
  -ComponentPackageMaterializationRoot C:\shared\src\incursa\.artifacts\grpc-unary-breadth-materialized-v2 `
  -RunIdPrefix grpc-unary-breadth-clean-v2 `
  -Output C:\shared\src\incursa\.artifacts\grpc-unary-breadth-runner-evidence `
  -Configuration Debug -NoRestore -FailOnError
```
