...
```text
org.protocol-lab.components.executor.quic-go-raw-load.0.1.17.<rid>.plabpkg
```
...
Version `0.1.17` adds explicit duplex sent, received, and aggregate byte/throughput metrics while retaining `throughputBytesPerSecond` as the legacy larger-direction compatibility metric.
Version `0.1.16` adds fixed-total 16MiB upload and download dispatch with exact 16,384x1KiB application-write shapes and strict scenario shape validation.
