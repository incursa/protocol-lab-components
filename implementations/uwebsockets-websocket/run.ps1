[CmdletBinding()]
param(
    [string]$Image = 'incursa-protocol-lab-uwebsockets-websocket:0.1.0',
    [int]$Port = 18085,
    [switch]$SkipBuild,
    [switch]$PlanOnly,
    [switch]$ProofOnly,
    [string]$OutputRoot = 'artifacts/uwebsockets-websocket'
)
...
$buildArgs = @('build','--pull','-f','docker/UWebSockets.Dockerfile','-t',$Image,'docker')
...
try {
    ...
    $version = (& docker @proofArgs 2>&1 | Out-String).Trim(); if ($LASTEXITCODE -ne 0 -or $version -notmatch 'uWebSockets 20\.79\.0') { throw "uWebSockets version proof failed: $version" }
