...
try {
    ...
    foreach ($scenario in $scenarios) {
        ...
        if ($result.executorId -ne 'go-grpc-h2-executor' -or $result.executorVersion -ne '0.4.0' -or
            $result.loadGeneratorId -ne 'go-x-net-http2-grpc-load' -or $result.loadGeneratorVersion -ne '0.4.0') {
