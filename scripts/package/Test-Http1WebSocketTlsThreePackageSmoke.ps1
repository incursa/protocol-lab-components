...
$scenarioPackage = Get-ChildItem $OutputRoot -File -Filter 'org.protocol-lab.components.scenario.http1-websocket-tls-performance.0.1.0.plabpkg' | Select-Object -First 1
$executorPackage = Get-ChildItem $OutputRoot -File -Filter 'org.protocol-lab.components.executor.go-http1-websocket-tls-executor.0.1.0.win-x64.plabpkg' | Select-Object -First 1
$targetPackage = Get-ChildItem $OutputRoot -File -Filter 'org.protocol-lab.components.implementation.go-http1-websocket-tls.0.1.0.win-x64.plabpkg' | Select-Object -First 1
...
try {
    ...
    $scenarioIds=@('http1.websocket.rfc6455.tls.upgrade','http1.websocket.rfc6455.tls.control-frames','http1.websocket.rfc6455.tls.text-echo','http1.websocket.rfc6455.tls.binary-echo','http1.websocket.rfc6455.tls.close')
    $results=@()
    foreach ($scenarioId in $scenarioIds) {
        ...
        if ($result.status -ne 'passed' -or $result.scenarioId -ne $scenarioId -or $result.executor.id -ne 'go-http1-websocket-tls-executor' -or $result.executor.version -ne '0.1.0' -or $result.loadGenerator.id -ne 'go-http1-websocket-tls-load' -or $result.loadGenerator.version -ne '0.1.0' -or $result.protocolProof.requestedProtocol -ne 'websocket-over-h1-tls' -or $result.protocolProof.observedProtocol -ne 'websocket-over-h1-tls' -or $result.protocolProof.protocolVariant -ne 'websocket-h1-tls1.3-upgrade' -or $result.protocolProof.fallbackDetected -ne $false -or $result.protocolProof.tls.version -ne 'TLS 1.3' -or $result.protocolProof.tls.alpn -ne 'http/1.1' -or $result.protocolProof.tls.serverName -ne 'websocket.plab.test' -or $result.protocolProof.tls.didResume -ne $false -or $result.protocolProof.tls.earlyData -ne $false -or $result.protocolProof.tls.verifiedChainCount -lt 1 -or $result.metrics.completedOperations -le 0 -or $result.metrics.failedOperations -ne 0 -or $result.metrics.timedOutOperations -ne 0) { throw "$scenarioId normalized evidence failed validation." }
    ...
    $unsupportedIds=@('websocket.echo','http1.websocket.rfc6455.cleartext.upgrade','http1.websocket.rfc6455.cleartext.control-frames','http1.websocket.rfc6455.cleartext.text-echo','http1.websocket.rfc6455.cleartext.binary-echo','http1.websocket.rfc6455.cleartext.close','http1.websocket.rfc6455.tls.subprotocol-text-echo','http1.websocket.rfc6455.tls.permessage-deflate-binary-echo','http2.websocket.rfc8441.extended-connect','http2.websocket.rfc8441.control-frames','http2.websocket.rfc8441.text-echo','http2.websocket.rfc8441.binary-echo','http2.websocket.rfc8441.close','http2.websocket.rfc8441.multi-message-text-echo','http3.websocket.rfc9220.extended-connect','http3.websocket.rfc9220.control-frames','http3.websocket.rfc9220.text-echo','http3.websocket.rfc9220.binary-echo','http3.websocket.rfc9220.close','http3.websocket.rfc9220.fragmented-binary-echo')
    $unsupportedProbes=@()
