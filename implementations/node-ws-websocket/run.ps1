[CmdletBinding()]
param(
    [string]$Image = 'incursa-protocol-lab-node-ws-websocket:0.1.1',
    [int]$Port = 18083,
    [switch]$SkipBuild,
    [switch]$PlanOnly,
    [switch]$ProofOnly,
    [string]$OutputRoot = 'artifacts/node-ws-websocket'
)
...
$buildArgs = @('build','--pull','-f','docker/NodeWs.Dockerfile','-t',$Image,'docker')
...
try {
    ...
    $version = (& docker @proofArgs 2>&1 | Out-String).Trim(); if ($LASTEXITCODE -ne 0 -or $version -notmatch 'ws 8\.21\.0') { throw "Node ws version proof failed: $version" }
