...
param(
    ...
    [string]$Image = 'incursa-protocol-lab-aioquic-rfc9220-websocket:0.3.1',
    [ValidateSet('origin-server', 'proxy')]
    [string]$ImplementationRole = $(if ($env:PLAB_IMPLEMENTATION_ROLE) { $env:PLAB_IMPLEMENTATION_ROLE } else { 'origin-server' }),
...
function Write-TerminalResult([string]$status, [string]$reason) {
    $value = [ordered]@{ schemaVersion = 'protocol-lab.rfc9220-executor-result.v1'; executorId = 'aioquic-rfc9220-websocket'; executorVersion = '0.3.1'; loadGeneratorId = 'aioquic-rfc9220-websocket-load'; loadGeneratorVersion = '0.3.1'; parserId = 'protocol-lab-rfc9220-json'; scenarioId = $ScenarioId; loadProfileId = $LoadProfileId; status = $status; passed = $false; reason = $reason }
...
try {
    ...
    if (-not $client.passed -or $client.status -ne 'passed' -or $client.scenarioId -ne $ScenarioId -or $client.loadProfileId -ne $LoadProfileId -or $client.executorId -ne 'aioquic-rfc9220-websocket' -or $client.executorVersion -ne '0.3.1' -or $client.loadGeneratorId -ne 'aioquic-rfc9220-websocket-load' -or $client.loadGeneratorVersion -ne '0.3.1' -or $client.parserId -ne 'protocol-lab-rfc9220-json' -or $client.implementationRole -ne $ImplementationRole) { throw 'client result identity, role, or validation status mismatch' }
