...
| Live controller aioquic 64 KiB proof | `job-80f48baab27248509a0513a80860f0d0`; current `0.3.4` canonical 64 KiB validation and measurement passed with 72 retained artifacts; publication `pub_8f8ae826bd3a437f88d2bd71afa535b5` uploaded and verified ten objects |
...
| Live controller quiche/ngtcp2 h3spec/QPACK proof | `job-14c77c9f080840389da6a733c648c3f7`; ngtcp2 `0.1.9` passed 15/15 cases, while quiche `0.1.10` passed 13/15 and failed RFC 9204 Sections 4.1.3 and 4.4.3; diagnostic report `pub_d441ef838c9b4d7d831bfa8f71743702` is uploaded, verified, and live with zero accepted performance measurements |
...
| --- | --- | --- |
| `kestrel-http3` | source package present | self-contained Linux x64 target package; current `0.1.8` has published h3spec status, response-header, and QPACK proof; `0.1.6` retains historical payload proof |
| `caddy-http3` | visible on public site and present in internal consumer manifests, but not packaged in this repo | current `0.1.9` has published h3spec status, response-header, and QPACK proof; `0.1.7` retains historical payload proof |
| `nginx-http3` | present in internal consumer manifests, but not packaged in this repo | current `0.1.9` has published h3spec status, response-header, and QPACK proof; `0.1.7` retains historical payload proof |
...
| `aioquic-http3` | source package present | current `0.3.4` package has live canonical 64 KiB proof; `0.3.3` retains historical h3spec status, response-header, and QPACK proof; `0.3.2` retains historical canonical status and 1KB proof |
| `quiche-http3` | source package present | current `0.1.10` has published h3spec/QPACK proof: 13/15 conformance cases passed and two RFC 9204 cases failed; historical `0.1.9` retains peer-characterization proof |
| `ngtcp2-http3` | source package present | current `0.1.9` has published h3spec/QPACK proof with 15/15 conformance cases passing; historical `0.1.8` retains peer-characterization proof |
...
| HTTP/3 WebSocket scenario pack | missing | immutable corrected `org.protocol-lab.components.scenario.http3-websocket-performance@0.2.3` authority-locks the six RFC9220 v2 scenarios and exact `websocket-smoke`/`diagnostic` profiles |
...
| `s2n-tls13` | missing | comparable package `org.protocol-lab.components.implementation.s2n-tls13@0.1.0`; source-built s2n-tls 1.7.5 with a minimal canonical full-handshake adapter, exact post-handshake profile validation, and no application-data surface |
| `wolfssl-tls13` | missing | comparable package `org.protocol-lab.components.implementation.wolfssl-tls13@0.1.0`; source-built wolfSSL 5.9.2 upstream server constrained to the canonical full-handshake controls, with the GPL-3.0 source and license retained through the Docker build |
...
| Technitium classic authoritative DNS | missing | source/config-only `org.protocol-lab.components.implementation.technitium-classic-authority@0.1.1`; `0.1.0` real-controller job `job-c92cc1e276e14a4c83cfb8ef396e19a4` proved the default `/32` QPM limit rejects the calibration load, while `0.1.1` disables IPv4/IPv6 QPM prefix limits through the supported API and restores exact UDP/TCP rows pending fresh proof |

## Scenario Support

| Package | Raw QUIC | H3 1KB | H3 64KB | H3 large body | Header-heavy / QPACK | WebSocket-over-H3 | h3spec / QPACK executor |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `kestrel-http3` | unsupported | supported, historical 0.1.6 proof | supported, historical 0.1.6 proof | supported metadata | supported, current live proof | unsupported | compatible target, current live proof |
| `caddy-http3` | unsupported | supported, historical 0.1.7 proof | supported, historical 0.1.7 proof | unsupported | supported, current live proof | unsupported | compatible target, current live proof |
| `nginx-http3` | unsupported | supported, historical 0.1.7 proof | supported, historical 0.1.7 proof | skipped pending broader fixture promotion | supported, current live proof | unsupported | compatible target, current live proof |
| `aioquic-http3` | unsupported | supported, historical 0.3.2 proof | supported, live 0.3.4 proof | unsupported | supported, historical 0.3.3 proof | supported via RFC9220 endpoint and executor | compatible target, historical 0.3.3 proof |
| `quiche-http3` | unsupported | validation-failed; characterization supported | validation-failed; characterization supported | validation-failed; characterization supported | diagnostic h3spec proof: 13/15 cases passed, RFC 9204 Sections 4.1.3 and 4.4.3 failed | unsupported | compatible target, current live proof with explicit conformance failures |
| `ngtcp2-http3` | unsupported | validation-failed; characterization supported | validation-failed; characterization supported | validation-failed; characterization supported | diagnostic h3spec proof: 15/15 cases passed | unsupported | compatible target, current live proof |
| `quic-go-http3` | unsupported | supported, historical 0.1.6 proof | supported, historical 0.1.6 proof | supported metadata | supported, current live proof | unsupported | compatible target, current live proof |
...
| `org.protocol-lab.components.implementation.s2n-tls13` | `implementations/s2n-tls13` |
| `org.protocol-lab.components.implementation.wolfssl-tls13` | `implementations/wolfssl-tls13` |
...
| --- | --- | --- | --- |
| `org.protocol-lab.components.implementation.caddy-http3` | `0.1.9` | `ce9ab38f2a327224fc822846dc27cb7265e8f3e4dd41e102de0057595a2892d2` | admitted, installed, selectable; current h3spec status, exact response-header, and QPACK rows passed in `job-e05ec641965748f3830b5a006b3d8425` and are published; `0.1.7` payload proof remains historical |
| `org.protocol-lab.components.implementation.nginx-http3` | `0.1.9` | `6a1cab88f443ee9c4c299a35da5a5a9c031d98d5df21ac83ba80f08f24171f98` | admitted, installed, selectable; current h3spec status, exact response-header, and QPACK rows passed in `job-e05ec641965748f3830b5a006b3d8425` and are published; `0.1.7` payload proof remains historical |
| `org.protocol-lab.components.implementation.quic-go-http3` | `0.1.8` | `e79295e62690f334d6e80092a3d40dc7a98d5b83c51e9172852f78b4623314fe` | admitted, installed, selectable; current h3spec status, exact response-header, and QPACK rows passed in `job-e05ec641965748f3830b5a006b3d8425` and are published; `0.1.6` payload proof remains historical |
...
| `org.protocol-lab.components.implementation.aioquic-http3` | `0.3.4` | `54a071a366e6cef25c4b3fbaaa70004156e1a0e01dbf7ef0a534f244ac5c4088` | admitted, installed, selectable; current canonical 64 KiB validation and measurement passed in `job-80f48baab27248509a0513a80860f0d0`; h3spec proof remains retained on immutable `0.3.3` and canonical status/1KB proof on immutable `0.3.2` |
| `org.protocol-lab.components.implementation.kestrel-http3` | `0.1.8` | `887643bff19bd2c61d298913f4fb8465e9fb827116b428613092d0020f37f7ef` | admitted, installed, selectable; current h3spec status, exact response-header, and QPACK rows passed in `job-6c8c02669dbf423fbcc7a12a5d364741` and are published; `0.1.6` payload proof remains historical |
| `org.protocol-lab.components.implementation.quiche-http3` | `0.1.10` | `dae0e697c061eade0f3a4b88ba02beff36bed103791bb59bcc4b9d1379ef39d7` | admitted, installed, selectable; `job-14c77c9f080840389da6a733c648c3f7` completed the QPACK cell with 13/15 h3spec cases passing and explicit RFC 9204 Sections 4.1.3 and 4.4.3 failures; published diagnostic report has zero accepted performance measurements; historical `0.1.9` peer proof remains retained |
| `org.protocol-lab.components.implementation.ngtcp2-http3` | `0.1.9` | `9ef7042b95521eee0e1f0f532282ba63f23d19101d3a91281bc14446eb2b76e3` | admitted, installed, selectable; `job-14c77c9f080840389da6a733c648c3f7` completed the QPACK cell with 15/15 h3spec cases passing; published diagnostic report has zero accepted performance measurements; historical `0.1.8` peer proof remains retained |
...
The controller also contains earlier immutable uploads from the Caddy, nginx, quic-go, aioquic, Kestrel, quiche, and ngtcp2 correction sequences. Do not use superseded or overbroad uploads for ranking. Use Caddy `0.1.9`, nginx `0.1.9`, quic-go HTTP/3 `0.1.8`, Kestrel `0.1.8`, quiche `0.1.10`, and ngtcp2 `0.1.9` for current h3spec/QPACK rows; aioquic `0.3.3` remains the immutable h3spec/QPACK proof and `0.3.4` is the current 64 KiB payload proof. Use other cited older immutable versions only for retained payload or peer-characterization rows. The quiche current proof includes two explicit conformance failures and neither peer target has an accepted performance measurement.
