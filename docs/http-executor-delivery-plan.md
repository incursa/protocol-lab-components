...
The clean component integration head is `2f8a8fc`; the runner integration head is `9bf1cae`, including the integration-only TLS record load-shape repair at `7d34627` and exact DoQ admission at `9bf1cae`. The consolidated runner and CLI build with zero warnings and errors, and the combined TLS, gRPC, secure-DNS, and WebSocket parser/admission/writer selection passes `249/249` focused tests. The component integration validates `54` public and `54` internal manifests; focused race tests and `go vet` pass for every consolidated Go executor and target.
