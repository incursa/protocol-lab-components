...
The controller also contains earlier immutable uploads from the Caddy, nginx, quic-go, aioquic, Kestrel, quiche, and ngtcp2 correction sequences. Do not use superseded or overbroad uploads for ranking. Use Caddy `0.1.9`, nginx `0.1.9`, quic-go HTTP/3 `0.1.8`, aioquic `0.3.3`, and Kestrel `0.1.8` for current h3spec/QPACK rows; use their cited older immutable versions only for retained payload rows. Quiche `0.1.5`, ngtcp2 `0.1.4`, curl `0.1.4`, and `http3-peer-characterization` `0.1.0` are admitted/selectable, but their attempted characterization job produced no completed evidence.
