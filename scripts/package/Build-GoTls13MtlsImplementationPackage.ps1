...
if($LASTEXITCODE-ne 0){throw 'go-tls13-mtls tests failed.'}
$rid=switch($RuntimeIdentifier){
    'win-x64'{@{os='windows';arch='x64';goOs='windows';goArch='amd64';name='go-tls13-mtls.exe'}}
    'linux-x64'{@{os='linux';arch='x64';goOs='linux';goArch='amd64';name='go-tls13-mtls'}}
}
...
try{
    ...
    if($LASTEXITCODE-ne 0){throw "go-tls13-mtls build failed for $RuntimeIdentifier."}
}finally{$env:GOOS=$oldOS;$env:GOARCH=$oldArch}
Copy-Item (Join-Path $componentRoot 'protocol-lab-package.json') $packageRoot
Copy-Item (Join-Path $componentRoot 'implementations/go-tls13-mtls.yaml') (Join-Path $packageRoot 'implementations')
...
$implementationPath=Join-Path $packageRoot 'implementations/go-tls13-mtls.yaml'
