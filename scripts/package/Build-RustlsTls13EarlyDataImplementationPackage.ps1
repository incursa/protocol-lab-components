...
$componentName='rustls-tls13-early-data';$componentRoot=Join-Path $Root "implementations/$componentName";$sourceRoot=Join-Path $componentRoot 'source'
& cargo "+$Toolchain" test --locked --manifest-path (Join-Path $sourceRoot 'Cargo.toml')
if($LASTEXITCODE-ne 0){throw 'rustls TLS early-data target tests failed.'}
$rid=switch($RuntimeIdentifier){
    'win-x64'{@{os='windows';arch='x64';target=$null;name='rustls-tls13-early-data.exe';source='target/release/protocol-lab-rustls-tls13-early-data-target.exe'}}
    'linux-x64'{@{os='linux';arch='x64';target='x86_64-unknown-linux-musl';name='rustls-tls13-early-data';source='target/x86_64-unknown-linux-musl/release/protocol-lab-rustls-tls13-early-data-target'}}
}
...
try{
    ...
    & cargo @buildArgs
    if($LASTEXITCODE-ne 0){throw "rustls TLS early-data target build failed for $RuntimeIdentifier."}
...
Copy-Item -LiteralPath (Join-Path $componentRoot 'implementations/rustls-tls13-early-data.yaml') -Destination (Join-Path $packageRoot 'implementations')
...
Copy-Item -LiteralPath (Join-Path $componentRoot 'certs/leaf.pem') -Destination (Join-Path $packageRoot 'certs')
Copy-Item -LiteralPath (Join-Path $componentRoot 'certs/leaf-key.pem') -Destination (Join-Path $packageRoot 'certs')
