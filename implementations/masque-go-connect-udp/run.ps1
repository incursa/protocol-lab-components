[CmdletBinding()]
param([string]$Image='incursa-protocol-lab-masque-go-connect-udp:0.1.1',[int]$Port=5471,[switch]$SkipBuild,[switch]$PlanOnly)
...
try{
  if($PlanOnly){[ordered]@{schemaVersion='protocol-lab.endpoint-plan.v1';implementationId='masque-go-connect-udp';packageVersion='0.1.1';upstreamVersion='v0.4.0';scenarioId=$scenario;image=$Image;hostPort=$Port;containerPort=4443;protocol='masque-connect-udp-over-h3';roles=@('proxy','udp-target')}|ConvertTo-Json -Compress;return}
  if(-not $SkipBuild){& docker build --pull -f docker/Dockerfile -t $Image .;if($LASTEXITCODE-ne 0){throw 'Docker build failed.'}}
  $version=(& docker run --rm $Image --version).Trim();if($version-ne'masque-go-connect-udp 0.1.1 masque-go v0.4.0'){throw "Version proof mismatch: $version"}
