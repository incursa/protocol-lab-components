...
Version `0.1.14` preserves the underlying transport read failure when a duplex
response is truncated, so diagnostics retain both the exact byte mismatch and
the QUIC read error. Version `0.1.13` fixes the public package declaration for the already-proven
