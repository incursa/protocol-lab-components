...
try {
    $executorArguments = @('--target-url', $TargetBaseUrl, '--output-dir', $OutputDirectory)
    if ([string]::IsNullOrWhiteSpace($env:PLAB_DURATION_SECONDS)) {
        $executorArguments += '--validation-only'
    }

    & go run . @executorArguments
