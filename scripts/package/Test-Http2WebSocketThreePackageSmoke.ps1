...
$targetArchive = Resolve-One "org.protocol-lab.components.implementation.kestrel-http2-websocket.0.1.0.$RuntimeIdentifier.plabpkg"
...
if ($scenarioManifest.packageVersion -ne '0.1.1' -or $executorManifest.packageVersion -ne '0.2.0' -or $targetManifest.packageVersion -ne '0.1.0') { throw 'immutable package version mismatch' }
...
try {
    ...
    if ($targetReady.implementationId -ne 'kestrel-http2-websocket' -or $targetReady.implementationVersion -ne '0.1.0' -or $targetReady.protocolVersion -ne 'HTTP/2' -or $targetReady.settingsEnableConnectProtocol -ne 1 -or $targetReady.alpn -ne 'h2' -or $targetReady.certificateDerSha256 -ne 'fe996190f39355e3cfc201cbb7e2cba962a701b94ed08ff49e68e830216d0109' -or $targetReady.certificateSpkiSha256 -ne 'c2440fbe955033f341ca625c1804e21b50066d952ab24a4b53007dc1cfbf410c') { throw 'target readiness mismatch' }
