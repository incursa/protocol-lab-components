...
| OpenSSL `s_server` | missing | wrapper-only TLS endpoint/tool package `org.protocol-lab.components.implementation.openssl-s-server@0.1.0`; exact OpenSSL 3.3.0 runtime gate and `tls.handshake.full` only; resumption, record, mTLS, early-data, and KeyUpdate rows remain explicit unsupported |
| GnuTLS `gnutls-serv` | missing | wrapper-only TLS endpoint/tool package `org.protocol-lab.components.implementation.gnutls-serv@0.1.0`; exact GnuTLS 3.8.9 Linux runtime gate and `tls.handshake.full` only; resumption, record, mTLS, early-data, and KeyUpdate rows remain explicit unsupported |
