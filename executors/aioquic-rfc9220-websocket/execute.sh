...
image="${AIOQUIC_RFC9220_WEBSOCKET_IMAGE:-incursa-protocol-lab-aioquic-rfc9220-websocket:0.3.1}"
implementation_role="${PLAB_IMPLEMENTATION_ROLE:-origin-server}"
...
case "$scenario_id" in
  ...
  websocket.echo|http1.websocket.*|http2.websocket.*) printf '{"schemaVersion":"protocol-lab.rfc9220-executor-result.v1","executorId":"aioquic-rfc9220-websocket","executorVersion":"0.3.1","scenarioId":"%s","status":"unsupported","passed":false}\n' "$scenario_id" | tee "$output_root/result.json"; exit 3 ;;
  *) printf '{"schemaVersion":"protocol-lab.rfc9220-executor-result.v1","executorId":"aioquic-rfc9220-websocket","executorVersion":"0.3.1","scenarioId":"%s","status":"unknown","passed":false}\n' "$scenario_id" | tee "$output_root/result.json"; exit 2 ;;
esac
[[ "$implementation_role" == origin-server || "$implementation_role" == proxy ]] || { printf 'PLAB_IMPLEMENTATION_ROLE must be origin-server or proxy\n' >&2; exit 1; }
