...
| Technitium classic authoritative DNS | missing | source/config-only `org.protocol-lab.components.implementation.technitium-classic-authority@0.1.0`; the package pins the upstream Linux amd64 image digest, denies recursion, creates the deterministic TTL-zero zone through the live API, and declares exact UDP pending live proof; TCP is unsupported because the reused-connection diagnostic ends in `EOF` |
