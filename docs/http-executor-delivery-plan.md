...
Explicit unsupported inventories remain fail closed. TLS: `tls.handshake.resumed`, `tls.0rtt`, `tls.record.throughput`, `tls.handshake.full.tls12`, `tls.handshake.full.chacha20`, `tls.handshake.mutual-auth`, `tls.early-data.accepted`, `tls.early-data.rejected`, `tls.key-update.diagnostic`, and `tls.record.coverage`. gRPC: `grpc.h2.bidi-streaming.echo`, `grpc.h2.client-cancellation`, `grpc.h2.client-streaming.echo`, `grpc.h2.deadline-exceeded`, `grpc.h2.server-streaming.echo`, `grpc.h2.trailers-only-status`, `grpc.h2.unary.echo-new-channel`, `grpc.h2.unary.empty`, `grpc.h2.unary.fixed-metadata`, `grpc.h2.unary.gzip`, and `grpc.h2.unary.large`. DNS: `dns.classic.tcp.query.a`, `dns.classic.udp-truncated-tcp-retry`, `dns.classic.udp.query.a`, `dns.doh2.query.a`, `dns.doh3.get.a`, `dns.doh3.query.a`, `dns.doh3.query.aaaa`, `dns.doh3.query.cname-chain`, `dns.doh3.query.large-dnssec-shaped`, `dns.doh3.query.nodata`, `dns.doh3.query.nxdomain`, and `dns.doq.query.a`.

Verification commands included `go test -race -count=1 ./...` and `go vet ./...` for the H1, H2, TLS, gRPC, and DoT Go packages; warning-as-error builds for the affected .NET runner, CLI, lab, worker, and TLS target projects; `Validate-ProtocolLabComponentManifests.ps1`; every affected package builder and build-attestation validator; the three direct package-backed runner commands; public repository health validation; Protocol Execution Result v2 JSON Schema plus artifact-hash validation; and `git diff --check`. Final package archives are dirty-source diagnostic artifacts and remain ineligible for publication.
