[CmdletBinding()]
param([string]$Image='incursa-protocol-lab-webtransport-go:0.1.2',[int]$Port=5461,[switch]$SkipBuild,[switch]$PlanOnly)
$ErrorActionPreference='Stop'
$scenario=if($env:PLAB_SCENARIO_ID){$env:PLAB_SCENARIO_ID}else{'webtransport.session-bidi-echo'}
if($scenario-notin@('webtransport.session-bidi-echo','webtransport.session-datagram-echo')){[ordered]@{schemaVersion='protocol-lab.unsupported.v1';status='unsupported';scenarioId=$scenario;implementationId='webtransport-go';supportedScenarios=@('webtransport.session-bidi-echo','webtransport.session-datagram-echo')}|ConvertTo-Json -Compress;exit 3}
Push-Location $PSScriptRoot
try{
  if($PlanOnly){[ordered]@{schemaVersion='protocol-lab.endpoint-plan.v1';implementationId='webtransport-go';packageVersion='0.1.2';upstreamVersion='v0.11.1';scenarioId=$scenario;image=$Image;hostPort=$Port;containerPort=4433;protocol='webtransport-over-h3'}|ConvertTo-Json -Compress;return}
  if(-not $SkipBuild){& docker build --pull -f docker/Dockerfile -t $Image .;if($LASTEXITCODE-ne 0){throw 'Docker build failed.'}}
  $version=(& docker run --rm $Image --version).Trim();if($version-ne'webtransport-go 0.1.2 webtransport-go v0.11.1'){throw "Version proof mismatch: $version"}
