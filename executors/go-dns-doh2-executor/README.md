...
Package version `0.2.1` executes strict `dns.doh2.query.a` and authoritative `dns.doh2.interoperability.query.a` with `secure-dns-smoke`. It establishes authenticated TLS with ALPN `h2`, reuses the connection for exact HTTP/2 `POST /dns-query`, rejects HTTP/1 fallback, and binds the reported protocol variant to the selected strict or interoperability scenario.
