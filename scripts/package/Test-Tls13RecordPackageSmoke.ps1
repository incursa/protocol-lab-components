...
foreach($cell in @(
    ...
    try{
        ...
        $env:PLAB_EXECUTOR_ID='go-tls13-executor';$env:PLAB_EXECUTOR_VERSION='0.3.1';$env:PLAB_LOAD_GENERATOR_ID='go-crypto-tls13-load';$env:PLAB_LOAD_GENERATOR_VERSION='0.3.1'
        ...
        if($result.scenarioId-ne $cell.scenario-or$result.executor.id-ne 'go-tls13-executor'-or$result.executor.version-ne '0.3.1'-or$result.loadGenerator.id-ne 'go-crypto-tls13-load'-or$result.loadGenerator.version-ne '0.3.1'-or$result.protocolProof.tlsVersion-ne 'TLS1.3'-or$result.protocolProof.alpn-ne 'protocol-lab-tls'-or$result.protocolProof.keyExchangeGroup-ne 'X25519'-or$result.protocolProof.cipherSuite-ne 'TLS_AES_128_GCM_SHA256'-or$result.protocolProof.earlyDataAttempted-or$result.metrics.completedOperations-lt 1-or$result.metrics.failedOperations-ne 0-or$result.metrics.timedOutOperations-ne 0){throw "$($cell.scenario) exact validity gate failed."}
