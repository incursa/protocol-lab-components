...
$packages = [ordered]@{
    scenario = Join-Path $PackageDirectory 'org.protocol-lab.components.scenario.grpc-h2-performance.0.3.0.plabpkg'
    executor = Join-Path $PackageDirectory 'org.protocol-lab.components.executor.go-grpc-h2-executor.0.3.0.win-x64.plabpkg'
    target = Join-Path $PackageDirectory 'org.protocol-lab.components.implementation.go-grpc-h2.0.3.0.win-x64.plabpkg'
}
...
try {
    ...
    foreach ($scenario in $scenarios) {
        ...
        $env:PLAB_EXECUTOR_VERSION = '0.3.0'
