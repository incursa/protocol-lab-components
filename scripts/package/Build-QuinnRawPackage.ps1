...
$sourceImplementationManifest = Join-Path $componentRoot 'implementations/quinn-raw.yaml'
...
foreach ($path in @($sourcePackageManifest, $sourceInternalManifest, $sourceImplementationManifest, $sourceRunSh, (Join-Path $sourceRoot 'Cargo.toml'), (Join-Path $sourceRoot 'Cargo.lock'))) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Required Quinn raw package input was not found: $path"
    }
}
...
if (-not $SkipSmoke) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Quinn raw cargo test failed with exit code $LASTEXITCODE."
    }
}

& docker run --rm --mount "type=bind,source=$dockerSource,target=/src" --workdir /src $buildImage cargo build --release --locked
if ($LASTEXITCODE -ne 0) {
    throw "Quinn raw cargo build failed with exit code $LASTEXITCODE."
}
...
Copy-Item -LiteralPath (Join-Path $sourceRoot 'target/release/protocol-lab-quinn-raw') -Destination (Join-Path $packageBinLinux 'quinn-raw') -Force
...
Copy-Item -LiteralPath $sourceImplementationManifest -Destination (Join-Path $packageImplementations 'quinn-raw.yaml') -Force
...
try {
    ...
    foreach ($entryName in @('README.md', 'bin/linux-x64/quinn-raw', 'implementations/quinn-raw.yaml', 'protocol-lab-package.json', 'protocol-lab.internal.json', 'run.sh')) {
