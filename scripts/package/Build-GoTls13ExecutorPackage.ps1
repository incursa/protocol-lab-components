...
$rid = switch ($RuntimeIdentifier) {
    'win-x64' { @{ os='windows'; arch='x64'; goOs='windows'; goArch='amd64'; name='go-tls13-executor.exe' } }
    'linux-x64' { @{ os='linux'; arch='x64'; goOs='linux'; goArch='amd64'; name='go-tls13-executor' } }
}
$stagingRoot = Join-Path $OutputRoot "go-tls13-executor/$RuntimeIdentifier"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "go-tls13-executor build failed with exit code $LASTEXITCODE." }
...
Copy-Item (Join-Path $componentRoot 'test-executors/go-tls13-executor.yaml') (Join-Path $packageRoot 'test-executors')
