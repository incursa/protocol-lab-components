...
| `aioquic-http3` | source package present | current `0.3.2` package has live status and 1KB proof |
...
| `aioquic-http3` | unsupported | supported, live proof | unproven | unsupported | supported metadata | supported via RFC9220 endpoint and executor | compatible target, unproven |
...
| `org.protocol-lab.components.implementation.aioquic-http3` | `0.3.2` | `95f53fa396fc63d7c3185a492906f88d26b9208545e879b0cdb550082fa42af8` | admitted, installed, selectable; current H3 status and 1KB validation and benchmarks passed in `job-0d08b2ace1704d609ec9803e6e7119c7` |
...
The controller also contains earlier immutable uploads from the Caddy, nginx, quic-go, aioquic, Kestrel, quiche, and ngtcp2 correction sequences. Do not use superseded or overbroad uploads for ranking. Use Caddy `0.1.7`, nginx `0.1.7`, quic-go HTTP/3 `0.1.6`, aioquic `0.3.2`, and Kestrel `0.1.6` only for the completed rows cited above. Quiche `0.1.5`, ngtcp2 `0.1.4`, curl `0.1.4`, and `http3-peer-characterization` `0.1.0` are admitted/selectable, but their attempted characterization job produced no completed evidence.
